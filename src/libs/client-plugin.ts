 ...
 */
export async function callExternalWithArgs(
  method: string,
  args?: Array<{}>
): Promise<CallExternalResult>;
/**
 * 调用外部接口（args形式），有回调
 *
 * @author
...
export async function callExternalWithArgs<TResult>(
  ...
  hasCallback: true,
  args?: Array<{}>
): Promise<CallExternalResultWithCallback<TResult>>;
export async function callExternalWithArgs<TResult>(
  method: string,
  arg0?: true | Array<{}>,
  arg1?: Array<{}>
  ...
  if (hasCallback) {
    ...
    return new Promise<CallExternalResultWithCallback<TResult>>(
      (resolve, reject) => {
